...
ConVar nav_coplanar_slope_limit("nav_coplanar_slope_limit", "0.99",
		FCVAR_CHEAT);
ConVar nav_coplanar_slope_limit_displacement(
		"nav_coplanar_slope_limit_displacement", "0.7", FCVAR_CHEAT);
ConVar nav_split_place_on_ground("nav_split_place_on_ground", "0",
		FCVAR_CHEAT,
		"If true, nav areas will be placed flush with the ground when split.");
ConVar nav_area_bgcolor("nav_area_bgcolor", "0 0 0 30", FCVAR_CHEAT,
		"RGBA color to draw as the background color for nav areas while editing.");
ConVar nav_corner_adjust_adjacent("nav_corner_adjust_adjacent", "18",
		FCVAR_CHEAT,
		"radius used to raise/lower corners in nearby areas when raising/lowering corners.");
ConVar nav_show_light_intensity("nav_show_light_intensity", "0",
		FCVAR_CHEAT);
ConVar nav_debug_blocked("nav_debug_blocked", "0", FCVAR_CHEAT);
ConVar nav_show_contiguous("nav_show_continguous", "0", FCVAR_CHEAT,
		"Highlight non-contiguous connections");

const float DEF_NAV_VIEW_DISTANCE = 1500.0;
ConVar nav_max_view_distance("nav_max_view_distance", "6000",
		FCVAR_CHEAT,
		"Maximum range for precomputed nav mesh visibility (0 = default 1500 units)");
ConVar nav_update_visibility_on_edit("nav_update_visibility_on_edit",
		"0", FCVAR_CHEAT,
		"If nonzero editing the mesh will incrementally recompue visibility");
ConVar nav_potentially_visible_dot_tolerance(
		"nav_potentially_visible_dot_tolerance", "0.98", FCVAR_CHEAT);
ConVar nav_show_potentially_visible("nav_show_potentially_visible", "0",
		FCVAR_CHEAT,
...
ConVar nav_selected_set_color("nav_selected_set_color", "255 255 200 96",
		FCVAR_CHEAT,
		"Color used to draw the selected set background while editing.", false,
		0.0f, false, 0.0f, SelectedSetColorChaged);
ConVar nav_selected_set_border_color("nav_selected_set_border_color",
		"100 100 0 255", FCVAR_CHEAT,
		"Color used to draw the selected set borders while editing.", false,
		0.0f, false, 0.0f, SelectedSetColorChaged);
...
inline bool CNavArea::IsVisible(const Vector &eye, Vector *visSpot) const {
	...
	MASK_BLOCKLOS_AND_NPCS | CONTENTS_IGNORE_NODRAW_OPAQUE, &traceFilter,
			&result);
 ...
 */
void CNavArea::Build(const Vector &nwCorner, const Vector &neCorner,
...
void CNavArea::build() {
	m_center = (m_nwCorner + m_seCorner) / 2.0f;
	if (m_seCorner.x - m_nwCorner.x > 0.0f
			&& m_seCorner.y - m_nwCorner.y > 0.0f) {
 ...
 */
void CNavArea::Build(CNavNode *nwNode, CNavNode *neNode, CNavNode *seNode,
		CNavNode *swNode) {
...
void CNavArea::GetNodes(NavDirType dir, CUtlVector<CNavNode *> *nodes) const {
	...
	CNavNode *node;
	for (node = m_node[startCorner]; node && node != m_node[endCorner]; node =
			node->GetConnectedNode(traversalDirection)) {
...
void CNavArea::ConnectTo(CNavArea *area, NavDirType dir) {
	...
	con.area = this;
	if (area->m_connect[dirOpposite].Find(con)
			== area->m_connect[dirOpposite].InvalidIndex()) {
...
void CNavArea::addLadder(CNavLadder* ladder,
		CNavLadder::LadderDirectionType dir) {
	Disconnect(ladder); // just in case
	NavLadderConnect tmp;
	tmp.ladder = ladder;
	m_ladder[dir].AddToTail(tmp);
}
...
public:
	LadderConnectionReplacement(CNavArea *originalArea,
			CNavArea *replacementArea) {
 ...
 */
void CNavArea::MergeAdjacentConnections(CNavMesh *TheNavNesh,
		CNavArea *adjArea) {
	// merge adjacency links - we gain all the connections that adjArea had
	int dir;
...
void CNavArea::AssignNodes(CNavArea *area) {
	...
	for (CNavNode *vertNode = m_node[NORTH_WEST];
			vertNode != m_node[SOUTH_WEST];
			vertNode = vertNode->GetConnectedNode(SOUTH)) {
		for (CNavNode *horizNode = vertNode; horizNode != horizLast;
...
public:
	SplitNotification(CNavArea *originalArea, CNavArea *alphaArea,
			CNavArea *betaArea) {
		m_originalArea = originalArea;
 ...
 */
bool CNavArea::SplitEdit(bool splitAlongX, float splitEdge, CNavArea **outAlpha,
		CNavArea **outBeta) {
	CNavArea *alpha = NULL;
	CNavArea *beta = NULL;
	...
	for (int dir = 0; dir < CNavLadder::NUM_LADDER_DIRECTIONS; ++dir) {
		FOR_EACH_VEC( m_ladder[ dir ], it )
		{
			...
			(alpha->GetDistanceSquaredToPoint(ladderPos)
					< beta->GetDistanceSquaredToPoint(ladderPos) ? alpha : beta)->ConnectTo(
					ladder);
		}
	}
 ...
 */
bool CNavArea::IsConnected(const CNavLadder *ladder,
...
bool CNavArea::IsConnected(const CNavArea *area, NavDirType dir) const {
	...
	if (dir == NUM_DIRECTIONS) {
		...
		{
			if (m_ladder[CNavLadder::LADDER_DOWN][dit].ladder->m_bottomArea
					== area)
				return true;
		}
	} else {
...
float CNavArea::ComputeGroundHeightChange(const CNavArea *area) {
	...
	float toZ, fromZ;
	return TheNavMesh->GetSimpleGroundHeight(closeTo + Vector(0, 0, StepHeight),
			&toZ)
 ...
 */
void CNavArea::AddIncomingConnection(CNavArea *source,
		NavDirType incomingEdgeDir) {
	NavConnect con;
	con.area = source;
	if (m_incomingConnect[incomingEdgeDir].Find(con)
			== m_incomingConnect[incomingEdgeDir].InvalidIndex()) {
...
void CNavArea::FinishSplitEdit(CNavArea *newArea, NavDirType ignoreEdge) {
		...
		for (int a = 0; a < count; ++a) {
			...
			case SOUTH:
				if (newArea->IsOverlappingX(adj)) {
					...
					if (adj->IsConnected(this,
							OppositeDirection((NavDirType) d)))
						adj->ConnectTo(newArea,
								OppositeDirection((NavDirType) d));
				}
				break;
	...
	if (HasNodes()) {
		...
		while (!newArea->IsOverlapping(
				*newArea->m_node[corner[0]]->GetPosition(),
				GenerationStepSize / 2)) {
			for (int i = 0; i < 2; ++i) {
				CNavNode*& node = newArea->m_node[corner[i]];
				Assert(node && node->GetConnectedNode( dir ));
				node = node->GetConnectedNode(dir);
			}
		}
...
void CNavArea::CalcDebugID() {
	if (m_debugid == 0) {
		...
		int coord[6] = { (int) m_nwCorner.x, (int) m_nwCorner.x,
				(int) m_nwCorner.z, (int) m_seCorner.x, (int) m_seCorner.y,
				(int) m_seCorner.z };
...
bool CNavArea::MergeEdit(CNavMesh* TheNavMesh, CNavArea *adj) {
	...
	build();
	m_neZ = m_seCorner.x > originalSECorner.x || m_nwCorner.y < originalNWCorner.y ?
		adj->GetZ(m_seCorner.x, m_nwCorner.y):
		GetZ(m_seCorner.x, m_nwCorner.y);

	m_swZ = m_nwCorner.x < originalNWCorner.x || m_seCorner.y > originalSECorner.y ?
		adj->GetZ(m_nwCorner.x, m_seCorner.y):
		GetZ(m_nwCorner.x, m_seCorner.y);
...
void CNavArea::InheritAttributes(CNavArea *first, CNavArea *second) {
	if (first && second) {
		...
		if (first->GetPlace() == second->GetPlace()) {
			...
			SetPlace(RandomInt(0, 100) < 50 ? first->GetPlace(): second->GetPlace());
		}
	} else if (first) {
...
bool CNavArea::Contains(const CNavArea *area) const {
	return (m_nwCorner.x <= area->m_nwCorner.x
			&& (m_seCorner.x >= area->m_seCorner.x)
			&& (m_nwCorner.y <= area->m_nwCorner.y)
			&& (m_seCorner.y >= area->m_seCorner.y)
			&& (m_nwCorner.z <= area->m_nwCorner.z)
			&& (m_seCorner.z >= area->m_seCorner.z));
}
...
float clampDim(float u) {
	u = fsel(u, u, 0);			// u >= 0 ? u : 0
	return fsel(u - 1.0f, 1.0f, u);	// u >= 1 ? 1 : u
}
...
float CNavArea::GetZ(float x, float y) const RESTRICT
{
	...
	float u = clampDim((x - m_nwCorner.x) * m_invDxCorners);
	float northZ = m_nwCorner.z + u * (m_neZ - m_nwCorner.z);
	return northZ + clampDim((y - m_nwCorner.y) * m_invDyCorners)
			* (m_swZ + u * (m_seCorner.z - m_swZ) - northZ);
}
 ...
 */
void CNavArea::GetClosestPointOnArea(const Vector * RESTRICT pPos,
		Vector *close) const RESTRICT {
	// Using fsel rather than compares, as much faster on 360 [7/28/2008 tom]
	float x = fsel(pPos->x - m_nwCorner.x, pPos->x, m_nwCorner.x);
	x = fsel(x - m_seCorner.x, m_seCorner.x, x);
	float y = fsel(pPos->y - m_nwCorner.y, pPos->y, m_nwCorner.y);
	y = fsel(y - m_seCorner.y, m_seCorner.y, y);

	close->Init(x, y, GetZ(x, y));
}
...
float CNavArea::GetDistanceSquaredToPoint(const Vector &pos) const {
	...
	float d = GetZ(pos) - pos.z;
	return d * d;
}

//--------------------------------------------------------------------------------------------------------------
CNavArea *CNavArea::GetRandomAdjacentArea(NavDirType dir) const {
	int which = RandomInt(0, m_connect[dir].Count() - 1);
 ...
 */
float CNavArea::ComputePortal(const CNavArea *to, NavDirType dir, Vector *center) const {
	float halfWidth = 0.0f;
	if (dir == NORTH || dir == SOUTH) {
		center->y = dir == NORTH ? m_nwCorner.y : m_seCorner.y;
	...
	{
		center->x = dir == WEST ? m_nwCorner.x: m_seCorner.x;
...
NavDirType CNavArea::ComputeLargestPortal(const CNavArea *to, Vector *center,
	...
	for (int i = 0; i < NUM_DIRECTIONS; ++i) {
		...
		float testHalfWidth = ComputePortal(to, testDir, &testCenter);
 ...
 */
void CNavArea::ComputeClosestPointInPortal(const CNavArea *to, NavDirType dir,
		const Vector &fromPos, Vector *closePos) const {
//	const float margin = 0.0f; //GenerationStepSize/2.0f;  // causes trouble with very small/narrow nav areas
	...
	if (dir == NORTH || dir == SOUTH) {
		closePos->y = dir == NORTH ? m_nwCorner.y: m_seCorner.y;
		float left = MAX(m_nwCorner.x, to->m_nwCorner.x);
		float right = MIN(m_seCorner.x, to->m_seCorner.x);
	...
	{
		closePos->x = dir == WEST ? m_nwCorner.x:  m_seCorner.x;
		float top = MAX(m_nwCorner.y, to->m_nwCorner.y);
		float bottom = MIN(m_seCorner.y, to->m_seCorner.y);
		...
		if (topMargin > bottomMargin) {
			// use midline
			bottomMargin = topMargin = (top + bottom) / 2.0f;
		}
		// limit y to within portal
		if (fromPos.y < topMargin) {
			closePos->y = topMargin;
		} else {
			closePos->y = fromPos.y > bottomMargin ? bottomMargin : fromPos.y;
		}
	}
...
bool CNavArea::IsContiguous(const CNavArea *other) const {
	...
	for (int dir = 0; dir < NUM_DIRECTIONS; ++dir) {
		if (IsConnected(other, (NavDirType) dir)) {
			Vector myEdge;
			ComputePortal(other, (NavDirType) dir, &myEdge);
			Vector otherEdge;
			other->ComputePortal(this, OppositeDirection((NavDirType) dir),
					&otherEdge);
 ...
 */
float CNavArea::ComputeAdjacentConnectionHeightChange(
		const CNavArea *destinationArea) const {
	VPROF_BUDGET("CNavArea::ComputeAdjacentConnectionHeightChange", "NextBot");
	// find which side it is connected on
	for (int dir = 0; dir < NUM_DIRECTIONS; ++dir) {
		if (IsConnected(destinationArea, (NavDirType) dir)) {
			Vector myEdge;
			ComputePortal(destinationArea, (NavDirType) dir, &myEdge);

			Vector otherEdge;
			destinationArea->ComputePortal(this, OppositeDirection((NavDirType) dir),
					&otherEdge);
...
bool CNavArea::IsEdge(NavDirType dir) const {
	FOR_EACH_VEC( m_connect[ dir ], it )
	{
		if (m_connect[dir][it].area->IsConnected(this, OppositeDirection(dir)))
			return false;
	}
...
NavDirType CNavArea::ComputeDirection(Vector *point) const {
	...
	if (fabs(to.x) > fabs(to.y)) {
		return to.x > 0.0f ? EAST : WEST;
	}
	return to.y > 0.0f ? SOUTH : NORTH;
}

//--------------------------------------------------------------------------------------------------------------
bool CNavArea::GetCornerHotspot(NavCornerType corner,
		Vector hotspot[NUM_CORNERS]) const {
	...
	return IntersectRayWithTriangle(ray, hotspot[0], hotspot[1], hotspot[2], false) > 0
			|| IntersectRayWithTriangle(ray, hotspot[2], hotspot[3], hotspot[0], false) > 0;
}
...
void CNavArea::Draw(void) const {
	...
	if (TheNavMesh->IsEditMode(CNavMesh::PLACE_PAINTING)) {
		...
		if (m_place == UNDEFINED_PLACE) {
			...
			color = TheNavMesh->GetNavPlace() == m_place ? NavSamePlaceColor : NavDifferentPlaceColor;
		}
	} else {
		// normal edit mode
		if (this == TheNavMesh->GetMarkedArea()) {
			...
			color = this == TheNavMesh->GetSelectedArea() ? NavSelectedColor : NavNormalColor;
		}
	}
	...
	if (IsDegenerate()) {
		...
		Text(GetCenter(), UTIL_VarArgs("Degenerate area %d", GetID()), true,
				DebugDuration);
	}
	...
	if (nav_show_light_intensity.GetBool()) {
		for (int i = 0; i < NUM_CORNERS; ++i) {
			...
			debugoverlay->AddLineOverlay(end, pos, lightIntensity,
					lightIntensity, MAX(192, lightIntensity), true,
					DebugDuration);
		}
	}
	...
	if (4
		...
		if (bgcolor[3] > 0) {
			...
			debugoverlay->AddTriangleOverlay(nw + offset, se + offset,
					ne + offset, bgcolor[0], bgcolor[1], bgcolor[2], bgcolor[3],
					true, DebugDuration);
			debugoverlay->AddTriangleOverlay(se + offset, nw + offset,
					sw + offset, bgcolor[0], bgcolor[1], bgcolor[2], bgcolor[3],
					true, DebugDuration);
		}
	}
	...
	void (*draw)(const Vector& from, const Vector& to, NavEditColor navColor)
			= (GetAttributes() & NAV_MESH_TRANSIENT) ?
					NavDrawDashedLine: NavDrawLine;
	draw(nw, ne, color);
	draw(ne, se, color);
	draw(se, sw, color);
	draw(sw, nw, color);
	if (this == TheNavMesh->GetMarkedArea()
			&& TheNavMesh->m_markedCorner != NUM_CORNERS) {
	...
	if (this != TheNavMesh->GetMarkedArea()
			&& this == TheNavMesh->GetSelectedArea()
			&& TheNavMesh->IsEditMode(CNavMesh::NORMAL)) {
		Vector p[NUM_CORNERS];
		if (GetCornerHotspot(GetCornerUnderCursor(), p)) {
	...
	if (GetAttributes() & NAV_MESH_STAIRS) {
		...
		// delta east west
		if (fabs(((GetCorner(NORTH_WEST).z + GetCorner(SOUTH_WEST).z
				- (GetCorner(NORTH_EAST).z + GetCorner(SOUTH_EAST).z)) /2.0f))
				// delta north south
				>  fabs(((GetCorner(NORTH_WEST).z + GetCorner(NORTH_EAST).z)
						 - (GetCorner(SOUTH_WEST).z + GetCorner(SOUTH_EAST).z)) / 2.0f)) {
	...
	if (GetAttributes() & NAV_MESH_STOP) {
		...
		NavDrawLine(m_center + Vector(dist, -length, 0),
				m_center + Vector(dist, length, 0), color);
		NavDrawLine(m_center + Vector(dist, length, 0),
				m_center + Vector(length, dist, 0), color);
		NavDrawLine(m_center + Vector(-dist, -length, 0),
				 m_center + Vector(-dist, length, 0), color);
		NavDrawLine(m_center + Vector(-dist, length, 0),
				m_center + Vector(-length, dist, 0), color);
		NavDrawLine(m_center + Vector(-length, dist, 0),
				 m_center + Vector(length, dist, 0), color);
		NavDrawLine(m_center + Vector(-dist, -length, 0),
				m_center + Vector(-length, -dist, 0), color);
		NavDrawLine(m_center + Vector(-length, -dist, 0),
	...
	if (GetAttributes() & NAV_MESH_WALK) {
		...
		float size = 8.0f;
		NavDrawHorizontalArrow(m_center + Vector(-size, 0, 0),
				m_center + Vector(size, 0, 0), 4, color);
	}
	...
	if (GetAttributes() & NAV_MESH_RUN) {
		...
		float size = 8.0f;
		float dist = 4.0f;
		NavDrawHorizontalArrow(m_center + Vector(-size, dist, 0),
				m_center + Vector(size, dist, 0), 4, color);
		NavDrawHorizontalArrow(m_center + Vector(-size, -dist, 0),
				m_center + Vector(size, -dist, 0), 4, color);
	}
	...
	if (GetAttributes() & NAV_MESH_AVOID) {
		...
		NavDrawTriangle(m_center, m_center + Vector(-topWidth, topHeight, 0),
				m_center + Vector(+topWidth, topHeight, 0), color);
		NavDrawTriangle(m_center + Vector(0, -bottomHeight, 0),
				m_center + Vector(-bottomWidth, -bottomHeight * 2, 0),
				m_center + Vector(bottomWidth, -bottomHeight * 2, 0), color);
	}
	...
	if (IsBlocked( TEAM_ANY) || HasAvoidanceObstacle() || IsDamaging()) {
		NavEditColor color =
				(IsBlocked( TEAM_ANY)
						...
						NavBlockedByFuncNavBlockerColor : NavBlockedByDoorColor;
...
void CNavArea::DrawSelectedSet(const Vector &shift) const {
	drawSelectionSet(GetCorner(NORTH_WEST) + shift,
			...
			GetCorner(SOUTH_EAST) + shift, s_selectedSetColor,
...
void CNavArea::DrawHidingSpots(void) const {
	...
	FOR_EACH_VEC( (*hidingSpots), it )
	{
		...
		if (spot->IsIdealSniperSpot()) {
			...
			color = spot->HasGoodCover() ? NavGoodCoverColor : NavExposedColor;
		}

		NavDrawLine(spot->GetPosition(), spot->GetPosition() + Vector(0, 0, 50),
				color);
	}
}
...
void CNavArea::DrawConnectedAreas(CNavMesh* TheNavMesh) const {
	...
	FOR_EACH_VEC( m_ladder[ CNavLadder::LADDER_UP ], it )
	{
		...
		if (!ladder->IsConnected(this, CNavLadder::LADDER_DOWN)) {
			NavDrawLine(m_center,
					ladder->m_bottom + Vector(0, 0, GenerationStepSize),
	...
	for (i = 0; i < NUM_DIRECTIONS; ++i) {
		...
		for (int a = 0; a < count; ++a) {
			...
			if (!TheNavMesh->IsEditMode(CNavMesh::PLACE_PAINTING)) {
				...
				NavEditColor color = adj->IsConnected(this, OppositeDirection(dir))?
						NavConnectedTwoWaysColor: NavConnectedOneWayColor;
				if (nav_show_contiguous.GetBool()) {
					color = IsContiguous(adj) ?
							NavConnectedContiguous : NavConnectedNonContiguous;
				}
				NavDrawLine(from, drawTo, color);
			}
		}
	}
}
...
void CNavArea::AddToOpenList(void) {
	Assert(
			(m_openList && m_openList->m_prevOpen == NULL) || m_openList == NULL);
...
bool CNavArea::IsHidingSpotCollision(const Vector &pos) const {
	...
	{
		if (( m_hidingSpots[it]->GetPosition() - pos).IsLengthLessThan(collisionRange))
			return true;
	}
...
bool IsHidingSpotInCover(const Vector &spot) {
	...
	UTIL_TraceLine(from, from + Vector(0, 0, 20.0f), MASK_NPCSOLID_BRUSHONLY,
	...
	for (float angle = 0.0f; angle < 2.0f * M_PI; angle += inc) {
		UTIL_TraceLine(from,
				from + Vector(coverRange * (float) cos(angle),
						coverRange * (float) sin(angle),
						HalfHumanHeight), MASK_NPCSOLID_BRUSHONLY, NULL,
...
static Vector FindPositionInArea(CNavArea *area, NavCornerType corner) {
	...
	if (!area->IsOverlapping(pos)) {
		...
		pos = cornerPos
				+ Vector(offset * multX, area->GetSizeY() * 0.5f * multY, 0.0f);
		if (!area->IsOverlapping(pos)) {
			// Try pulling the X offset to the area's center
			pos = cornerPos
					+ Vector(area->GetSizeX() * 0.5f * multX, offset * multY,
							0.0f);
			if (!area->IsOverlapping(pos)) {
				// Try pulling the X and Y offsets to the area's center
				pos = cornerPos
						+ Vector(area->GetSizeX() * 0.5f * multX,
								area->GetSizeY() * 0.5f * multY, 0.0f);
				if (!area->IsOverlapping(pos)) {
					AssertMsg(false,
							"A Hiding Spot can't be placed on its area at (%.0f %.0f %.0f)",
...
void CNavArea::ComputeHidingSpots(void) {
	...
	for (int d = 0; d < NUM_DIRECTIONS; ++d) {
		...
		FOR_EACH_VEC( m_connect[d], it )
		{
			...
			// ignore it
			if (!connect.area->IsConnected(this,
					OppositeDirection(static_cast<NavDirType>(d)))
			// ignore jump areas
...
void ClassifySniperSpot(HidingSpot *spot) {
	...
	FOR_EACH_VEC( TheNavAreas, it )
	{
		...
		Vector walkable;
		// scan this area
		for (walkable.y = areaExtent.lo.y + GenerationStepSize / 2.0f;
				walkable.y < areaExtent.hi.y; walkable.y +=
						GenerationStepSize) {
			for (walkable.x = areaExtent.lo.x + GenerationStepSize / 2.0f;
					walkable.x < areaExtent.hi.x; walkable.x +=
							GenerationStepSize) {
				walkable.z = area->GetZ(walkable) + HalfHumanHeight;

				// check line of sight
				UTIL_TraceLine(eye, walkable,
	...
	if (found) {
		...
		spot->m_flags |= (
				sniperExtent.Area() >= minIdealSniperArea
						|| farthestRangeSq >= longSniperRangeSq ?
...
void CNavArea::ComputeSniperSpots(void) {
	...
	FOR_EACH_VEC( m_hidingSpots, it )
	{
		ClassifySniperSpot(m_hidingSpots[it]);
	}
}
 ...
 */
SpotEncounter *CNavArea::GetSpotEncounter(const CNavArea *from,
		const CNavArea *to) {
	if (from && to) {
		FOR_EACH_VEC( m_spotEncounters, it )
		{
			SpotEncounter* e = m_spotEncounters[it];
 ...
 */
void CNavArea::AddSpotEncounters(const CNavArea *from, NavDirType fromDir,
		const CNavArea *to, NavDirType toDir) {
	...
	e->path.from.z = from->GetZ(e->path.from) + HumanEyeHeight;
	e->path.to.z = to->GetZ(e->path.to) + HumanEyeHeight;
	...
	bool done = false;
	for (float along = 0.0f; !done; along += 25.0f) {
		...
		Vector eye = e->path.from + along * dir;
		...
		FOR_EACH_VEC( TheHidingSpots, it )
		{
			...
			Vector delta(spotPos.x - eye.x, spotPos.y - eye.y,
					spotPos.z + HumanEyeHeight - eye.z);
			if (!spot->HasGoodCover() || spot->IsMarked()
					// check if in range
					|| delta.IsLengthGreaterThan(2000.0f))
				continue;

			// check if we have LOS
			// BOTPORT: ignore glass here
			UTIL_TraceLine(eye,
					Vector(spotPos.x, spotPos.y, spotPos.z + HalfHumanHeight),
			...
			if (dot < 0.7071f && dot > -0.7071f
				...
				e->spots.AddToTail();
				e->spots.Tail().t = along / length;
				e->spots.Tail().spot = spot;
			}
		...
		FOR_EACH_VEC( m_connect[ fromDir ], it )
		{
			...
			for (int toDir = 0; toDir < NUM_DIRECTIONS; ++toDir) {
				FOR_EACH_VEC( m_connect[ toDir ], ot )
				{
					...
					AddSpotEncounters(fromCon->area, (NavDirType) fromDir,
							toCon->area, (NavDirType) toDir);
				}
			}
		}
	}
...
void CNavArea::DecayDanger(void) {
	for (int i = 0; i < MAX_NAV_TEAMS; ++i) {
		m_danger[i] -= GetDangerDecayRate()
				* gpGlobals->curtime - m_dangerTimestamp[i];
		if (m_danger[i] < 0.0f)
			m_danger[i] = 0.0f;
...
float CNavArea::GetLightIntensity(const Vector &pos) const {
	Vector testPos(clamp(pos.x, m_nwCorner.x, m_seCorner.x),
			clamp(pos.y, m_nwCorner.y, m_seCorner.y), pos.z);
	float dX = (testPos.x - m_nwCorner.x) / (m_seCorner.x - m_nwCorner.x);
	float dY = (testPos.y - m_nwCorner.y) / (m_seCorner.y - m_nwCorner.y);

	return (m_lightIntensity[NORTH_WEST] + m_lightIntensity[SOUTH_WEST]
			+ m_lightIntensity[NORTH_EAST] * dX) * (1 - dY)
			+ m_lightIntensity[SOUTH_EAST] * dX * dY;
}
...
bool CNavArea::ComputeLighting(void) {
	...
	for (int i = 0; i < NUM_CORNERS; ++i) {
		...
		Vector light(0, 0, 0);
		Vector ambientColor;
		float ambientIntensity = ambientColor.x + ambientColor.y
				+ ambientColor.z;
...
void CNavArea::RaiseCorner(NavCornerType corner, int amount,
	...
	if (corner == NUM_CORNERS) {
		for (int i = 0; i < NUM_CORNERS; i++) {
			RaiseCorner(static_cast<NavCornerType>(i),
					amount, raiseAdjacentCorners);
		}
		return;
	}
	...
	if (!raiseAdjacentCorners
			|| nav_corner_adjust_adjacent.GetFloat() <= 0.0f) {
		return;
	}
	...
	for (int x = gridX - shift; x <= gridX + shift; ++x) {
		...
		for (int y = gridY - shift; y <= gridY + shift; ++y) {
			...
			NavAreaVector *areas = &TheNavMesh->m_grid[x
					+ y * TheNavMesh->m_gridSizeX];
			...
			FOR_EACH_VEC( (*areas), it )
			{
				...
				for (int i = 0; i < NUM_CORNERS; ++i) {
					Vector areaPos = area->GetCorner(NavCornerType(i));
 ...
 */
float FindGroundZ(const Vector& original, const Vector& corner1,
	...
	if (fabs(first - second) > StepHeight) {
		...
		return fabs(original.z - first) > fabs(original.z - second) ?
				second : first;
	}
...
void CNavArea::PlaceOnGround(NavCornerType corner, float inset) {
	...
	if (corner == NORTH_WEST || corner == NUM_CORNERS) {
		RaiseCorner(NORTH_WEST, FindGroundZ(nw, ne, sw) - nw.z);
	}
	if (corner == NORTH_EAST || corner == NUM_CORNERS) {
		RaiseCorner(NORTH_EAST, FindGroundZ(ne, nw, se) - ne.z);
	}
	if (corner == SOUTH_WEST || corner == NUM_CORNERS) {
		RaiseCorner(SOUTH_WEST, FindGroundZ(sw, nw, se) - sw.z);
	}
	if (corner == SOUTH_EAST || corner == NUM_CORNERS) {
		RaiseCorner(SOUTH_EAST, FindGroundZ(se, ne, sw) - se.z);
	}
}
...
static ConCommand nav_update_blocked("nav_update_blocked",
...
bool CNavArea::IsBlocked(int teamID, bool ignoreNavBlockers) const {
	...
	return m_isBlocked[teamID % MAX_NAV_TEAMS];
}

//--------------------------------------------------------------------------------------------------------
void CNavArea::MarkAsBlocked(int teamID, edict_t* blocker,
		bool bGenerateEvent) {
...
void CNavArea::UpdateBlockedFromNavBlockers(void) {
	...
	bool isBlocked = CFuncNavBlocker::CalculateBlocked(m_isBlocked, bounds.lo,
			bounds.hi);
	...
	if (wasBlocked != isBlocked) {
		...
		if (isBlocked) {
			...
			if (nav_debug_blocked.GetBool()) {
				ConColorMsg(Color(0, 128, 255, 255),
...
void CNavArea::UnblockArea(int teamID) {
	...
	if (teamID == TEAM_ANY) {
		...
		m_isBlocked[teamID % MAX_NAV_TEAMS] = false;
	}
	...
	if (wasBlocked) {
		...
		if (nav_debug_blocked.GetBool()) {
			ConColorMsg(Color(255, 0, 128, 255),
...
void CNavArea::UpdateBlocked(bool force, int teamID) {
	...
	CTraceFilterWalkableEntities filter( NULL, COLLISION_GROUP_PLAYER_MOVEMENT,
			WALK_THRU_DOORS | WALK_THRU_BREAKABLES);
#endif
	trace_t tr;
	{
		VPROF("CNavArea::UpdateBlocked-Trace");
		UTIL_TraceHull(origin, origin, bounds.lo, bounds.hi,
	...
	if (!tr.startsolid) {
		...
		if (teamID == TEAM_ANY) {
			...
			m_isBlocked[teamID % MAX_NAV_TEAMS] = true;
		}
	}
	...
	if (TheNavMesh->GetMarkedArea() == this) {
		bool blocked = IsBlocked(teamID);
		debugoverlay->AddBoxOverlay(origin, bounds.lo, bounds.hi,
				QAngle(0.0f, 0.0f, 0.0f), blocked ? 255 : 0, blocked ? 0 : 255, 0, 64, 3.0f);
	}
}
...
void CNavArea::CheckFloor(edict_t* ignore) {
	...
	// See if spot is valid
	trace_t tr;
	UTIL_TraceHull(origin, origin, Vector(-size, -size, 0),
			Vector(size, size, JumpCrouchHeight + 10.0f),
	MASK_NPCSOLID_BRUSHONLY,
			CTraceFilterSimple(ignore == nullptr ? nullptr: ignore->GetIServerEntity(),
...
void CNavArea::UpdateAvoidanceObstacles(void) {
	...
	for (int i = 0; i < TheNavMesh->GetObstructions().Count(); ++i) {
		...
		if (!IsBoxIntersectingBox(mins, maxs, vecSurroundMins, vecSurroundMaxs)
			...
			continue;
		obstructionHeight = MAX(obstructionHeight, obstruction->GetNavObstructionHeight());
	}
...
template<typename T>
bool CNavArea::HasFunc(void) const {
	for (int i = 0; i < m_funcNavCostVector.Count(); ++i) {
		if (dynamic_cast<T *>(m_funcNavCostVector[i].Get()) != nullptr) {
			return true;
		}
	}
	return false;
...
bool CNavArea::HasFuncNavAvoid(void) const {
	return HasFunc<CFuncNavAvoid>();
}

bool CNavArea::HasFuncNavPrefer(void) const {
	return HasFunc<CFuncNavPrefer>();
}
...
static void CommandNavCheckFloor(void) {
	...
	if (TheNavMesh->GetMarkedArea()) {
		...
		DevMsg("nav_check_floor took %2.2f ms\n",
				(Plat_FloatTime() - start) * 1000.0f);
	}
}
static ConCommand nav_check_floor("nav_check_floor",
...
static ConCommand nav_select_overlapping("nav_select_overlapping",
		CommandNavSelectOverlapping,
		"Selects nav areas that are overlapping others.", FCVAR_GAMEDLL);
...
void CNavArea::SetupPVS(void) const {
	...
	Vector shift(eye);
	for (shift.y = margin; shift.y <= GetSizeY() - margin; shift.y +=
			GenerationStepSize) {
		for (shift.x = margin; shift.x <= GetSizeX() - margin; shift.x +=
				GenerationStepSize) {
			// Optimization:
...
bool CNavArea::IsInPVS(void) const {
	Vector eye(0, 0, 0.75f * HumanHeight);
	Extent areaExtent;
	areaExtent.lo = areaExtent.hi = GetCenter() + eye;
	for (unsigned int corner = NORTH_WEST; corner < NUM_CORNERS; corner++) {
		areaExtent.Encompass(
				GetCorner(static_cast<NavCornerType>(corner)) + eye);
	}
	return engine->CheckBoxInPVS(areaExtent.lo, areaExtent.hi, m_PVS,
			m_nPVSSize);
}
 ...
 */
CNavArea::VisibilityType CNavArea::ComputeVisibility(const CNavArea *area,
		bool isPVSValid, bool bCheckPVS, bool *pOutsidePVS) const {
	...
	// limit range of visibility check
			&& distanceSq > Sqr(nav_max_view_distance.GetFloat())) {
	...
	vTraceMins.z = MIN(MIN( MIN( vThisNW.z, vThisNE.z ), vThisSE.z ),
			vThisSW.z);
	vTraceMaxs.z =
			MAX(MAX( MAX( vThisNW.z, vThisNE.z ), vThisSE.z ), vThisSW.z)
					+ 0.1;
	...
	UTIL_TraceHull(vThisCenter, vTarget, vTraceMins, vTraceMaxs,
			MASK_NAV_VISION, traceFilter, &tr);

	if (tr.fraction == 1.0
			|| (tr.endpos.x > vOtherMins.x && tr.endpos.x < vOtherMaxs.x
					&& tr.endpos.y > vOtherMins.y && tr.endpos.y < vOtherMaxs.y)) {
	...
	for (shift.y = margin; shift.y <= GetSizeY() - margin; shift.y +=
		...
		for (shift.x = margin; shift.x <= GetSizeX() - margin; shift.x +=
			...
			if (distanceSq > Sqr(1000)
					&& (testPos - (GetCenter() + eye)).Normalized().Dot(eyeToCenter)
					>= angleTolerance) {
				continue;
			}
...
void CNavArea::ComputeVisToArea(CNavArea *&pOtherArea) {
	...
	VisibilityType visThisToOther =
			(area == g_pCurVisArea) ? COMPLETELY_VISIBLE : NOT_VISIBLE;
	VisibilityType visOtherToThis = NOT_VISIBLE;

	if (area != g_pCurVisArea) {
		bool bOutsidePVS;

		visOtherToThis = g_pCurVisArea->ComputeVisibility(area, true, true,
				&bOutsidePVS); // TODO: Hacky right now. Compute visibility for the "complete" case actually returns how completely visible the area is to the other. Should fix it to be more clear [1/30/2009 tom]

		if (!bOutsidePVS
				&& (visOtherToThis
						|| (g_pCurVisArea->GetCenter() - area->GetCenter()).LengthSqr()
								< Sqr(nav_max_view_distance.GetFloat()))) {
			visThisToOther = area->ComputeVisibility(g_pCurVisArea, true,
					false);
		}
...
void CNavArea::ComputeVisibilityToMesh(void) {
	...
	g_pCurVisArea = this;
	ParallelProcess("CNavArea::ComputeVisibilityToMesh",
			collector.m_area.Base(), collector.m_area.Count(),
			&ComputeVisToArea);

	m_potentiallyVisibleAreas.EnsureCapacity(g_ComputedVis.Count());
	while (g_ComputedVis.Count()) {
		g_ComputedVis.PopItem(
				&m_potentiallyVisibleAreas[m_potentiallyVisibleAreas.AddToTail()]);
	}
	...
	FOR_EACH_VEC( collector.m_area, it )
	{
		...
		Assert(
				g_pNavVisPairHash->Find( visPair ) == g_pNavVisPairHash->InvalidHandle());
 ...
 */
bool CNavArea::IsEntirelyVisible(const Vector &eye,
		const edict_t *ignore) const {
	Vector corner;
	trace_t result;
	CTraceFilterNoNPCsOrPlayer traceFilter(ignore->GetIServerEntity(),
			COLLISION_GROUP_NONE);
	const float offset = 0.75f * HumanHeight;

	// check center
	UTIL_TraceLine(eye, GetCenter() + Vector(0, 0, offset), MASK_NAV_VISION,
			&traceFilter, &result);
	if (result.fraction < 1.0f) {
		return false;
	}
	...
	for (int c = 0; c < NUM_CORNERS; ++c) {
		corner = GetCorner((NavCornerType) c);
		UTIL_TraceLine(eye, corner + Vector(0, 0, offset), MASK_NAV_VISION,
				&traceFilter, &result);
		if (result.fraction < 1.0f) {
			return false;
		}
 ...
 */
bool CNavArea::IsPartiallyVisible(const Vector &eye,
		const edict_t *ignore) const {
	Vector corner;
	trace_t result;
	CTraceFilterNoNPCsOrPlayer traceFilter(
			ignore == nullptr ? nullptr : ignore->GetIServerEntity(),
			COLLISION_GROUP_NONE);
	const float offset = 0.75f * HumanHeight;
...
bool CNavArea::checkVisibility(const CNavArea *viewedArea,
	...
	for (int i = 0; i < m_potentiallyVisibleAreas.Count(); ++i) {
		if (m_potentiallyVisibleAreas[i].area == viewedArea) {
			...
			return visCheck(m_potentiallyVisibleAreas[i]);
		}
	}
...
template<typename VisCheck>
bool CNavArea::checkTeamVisibility(int teamIndex, const CNavArea* from,
		VisCheck visCheck) const {
	...
	for (int i = 0; i < gpGlobals->maxClients; ++i) {
		...
		if (pEnt) {
			...
			if (player->GetTeamIndex() == teamIndex && player->GetHealth() > 0
					&& from && (from->*visCheck)(this)) {
				return true;
			}
		}
	}
 ...
 */
bool CNavArea::IsPotentiallyVisibleToTeam(int teamIndex,
		const CNavArea* from) const {
	VPROF_BUDGET("CNavArea::IsPotentiallyVisibleToTeam", "NextBot");
	return checkTeamVisibility(teamIndex, from, &CNavArea::IsPotentiallyVisible);
}
 ...
 */
bool CNavArea::IsCompletelyVisibleToTeam(int teamIndex,
		const CNavArea* from) const {
	VPROF_BUDGET("CNavArea::IsCompletelyVisibleToTeam", "NextBot");
	return checkTeamVisibility(teamIndex, from, &CNavArea::IsCompletelyVisible);
}
