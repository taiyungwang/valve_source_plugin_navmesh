...
void CNavMesh::BuildLadders( void )
{
	...
	DestroyLadders();
	Vector mins, maxs;
	for (edict_t *ladder = findEntityByClassName(LADDER_NAME, nullptr); ladder!= nullptr;
			ladder = findEntityByClassName(LADDER_NAME, ladder)) {
...
void CNavMesh::AddWalkableSeeds( void )
{
	...
	{
		extern IPlayerInfoManager *playerinfomanager;
		IPlayerInfo *spawn = playerinfomanager->GetPlayerInfo(ent);
